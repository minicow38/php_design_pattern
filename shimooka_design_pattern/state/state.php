...
/*
 * MEMO
 * Contextが機能の実装、Stateが状態の実装を行う。「状態毎に複数の処理が切り替わる」というケースに出くわした場合→Stateパターンを疑ってみよう
 * 
 * 具体例として、「時間によりたくさんの処理が切り替わるような処理」を実装したい場合、Stateパターンが活きるはず
 * 
 * 他のパターンと併用したい場合、「Contextを委譲する」「Contextをインターフェースにして実装する」などの案が考えられる。
 * 
 * Stateパターンの以下の特徴に留意せよ
 * ・状態を追加するのはカンタン(各Stateクラスを微妙にいじらねばならないが)。
 * ・状態に依存した機能を追加するのは難しい(全てのStateクラスに、状態に依存した処理を追加する必要が出てくる)
 * 
 */
